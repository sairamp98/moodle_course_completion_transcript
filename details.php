...
// Print header.
$page = get_string('completionprogressdetails', 'block_cpm_completionstatus');
$title = format_string($course->fullname) . ': ' . $page;

$PAGE->navbar->add($page);
$PAGE->set_pagelayout('report');
$PAGE->set_url('/blocks/cpm_completionstatus/details.php', array('course' => $course->id, 'user' => $user->id));
$PAGE->set_title(get_string('course') . ': ' . $course->fullname);
$PAGE->set_heading($title);

// Adding the "Show User Dashboard" button.
$dashboard_url = new moodle_url('/blocks/cpm_completionstatus/user_dashboard.php', array('userid' => $user->id));
$button = html_writer::link($dashboard_url, get_string('showuserdashboard', 'block_cpm_completionstatus'), array('class' => 'btn btn-primary', 'style' => 'float: right; margin: 10px;'));
...
if (empty($completions)) {
    ...
    echo html_writer::tag('th', get_string('criteriagroup', 'block_cpm_completionstatus'), array('class' => 'c0 header', 'scope' => 'col'));
    echo html_writer::tag('th', get_string('criteria', 'completion'), array('class' => 'c1 header', 'scope' => 'col'));
    echo html_writer::tag('th', get_string('requirement', 'block_cpm_completionstatus'), array('class' => 'c2 header', 'scope' => 'col'));
